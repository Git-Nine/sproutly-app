...
const HEADER = `Sproutly plant-catalogue import — STAGING FILE (PROJ-11)
...
HOW TO REVIEW:
  ...
  3. Rows with \`review_required: true\` had a LOW-confidence survival-critical field and
     CANNOT be committed until you fix the value and set \`review_required: false\`.
  4. Rows with \`status: existing\` are already in the live catalogue — commit skips them
     (it never overwrites an admin's edits). Leave them or delete them.
  5. Set \`approved: true\` on every row you trust. Then run:  npm run import:plants:commit
