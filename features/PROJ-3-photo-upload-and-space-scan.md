...
Re-shaped `/scans/new` from a single long form (every field visible at once) into the prototype's three sequential screens (`docs/design-references/screen_02–04.png`), so the flow reads as **upload → read → confirm** instead of a wall of inputs. This lowers the entry barrier for the "Guilty Non-Starter" persona (one clear action per screen) and makes the AI vision prefill legible: the user *sees* "Reading your space…" happen, then reviews what was filled. Scope decided with the user: full 3-step wizard, **current data model only** — no new `orientation`/`maintenance` fields (the prototype's review card shows these, but adding them would need a DB migration + AI-prompt work; deferred). Frontend-only.

**What changed (frontend-only, no DB/API/env change):**
- `src/components/scans/scan-form.tsx` — the form is now a `Step` state machine (`upload | reading | review`):
  - **upload** ("Scan your space") — the big dashed photo dropzone + a "No photo handy? Continue without one" link that jumps straight to review (photo stays optional). Grounded copy replaces the prototype's aspirational "we'll demo with a sample garden" (no sample-garden feature exists).
  - **reading** ("Reading your space…") — shows the picked photo + a pulsing leaf while GPS auto-fill + the `POST /api/classify-vision` prefill run; auto-advances to review on completion (success *or* fallback).
  - **review** ("Here's what we see") — the photo as a clean hero image, conditions as design-system form rows (circular icon · taupe label · inline-editable control) via a new local `FieldRow`, the prefill hint, the **"Looks right — show me my plan"** CTA, and a **"Trouble reading the photo?"** disclosure that reveals the photo editor. Redirect target unchanged (`/scans/{short_code}`).
  - **Edit mode** (`scan !== null`) skips the wizard — opens directly on the review layout with the photo picker always visible and a "Save changes" CTA; replacing a photo re-classifies inline (no full-screen "reading" interstitial).
- `src/components/scans/photo-picker.tsx` — added a `variant="hero"` for the upload step's large "Point at your outdoor space / Tap to take or upload a photo" dropzone (empty state only); the compact preview+Replace/Remove editor is unchanged and reused on review/edit.
- `src/app/scans/new/page.tsx` — header now matches the prototype top bar: `← Back` · centered non-interactive logo · `1/4` step counter, with a 4-dot journey progress row (Scan → Plan → Order → Grow) under a hairline. The per-step `<h1>` moved into the wizard; the static intro was removed. (`ProfileLink` dropped from this screen per the prototype — reachable via Back → `/scans`.)
- **Tests:** `scan-form.test.tsx` updated for the staged flow (a `skipPhoto()` helper reaches review; prefill tests use `findByLabelText` since fields mount only after "reading"). Still 4 tests, full suite **194/194 green**, `tsc` + `npm run lint` + `npm run build` clean.
- **Status:** complete and live-ready — no env vars, no DB change, no ops steps. Ships on the next push to `main`. (The AI prefill it now showcases is still gated on the n8n instance + `N8N_CLASSIFY_*` vars per the earlier enhancement; the wizard degrades to the manual review form without them.)

## Post-Deploy Enhancement — Postcode auto-fill (remember last + auto-geolocate) (2026-07-06)

Recurring feedback that "the postcode still isn't auto-filled." Root cause is not a bug: the photo-GPS → postcode path only fires when the uploaded photo actually carries GPS EXIF, and most real uploads don't — screenshots, EXIF-stripped messenger shares, and (the big one for a mobile-first PWA) iOS Safari handing the web file input a location-stripped copy of a library photo. The `2026-07-02` "Use my location" button covered the skip-photo case but required a deliberate tap. This closes the gap by making the fill happen without one, in the two cases that actually recur. Frontend-only, **no DB change / no migration**.

**What changed:**
- **Remember the last postcode (`src/app/scans/new/page.tsx` + `scan-form.tsx`):** the new-scan page queries the user's most recent scan's non-null postcode (RLS-scoped to them, tolerant of the table being absent) and passes it to `ScanForm` as a new `defaultPostcode` prop. It pre-fills the field with a new `autofillSource: 'remembered'` → "From your last space — edit if needed" hint. Chosen over a `users.last_postcode` column precisely to avoid a migration (and any contention with concurrent DB work); "most recent scan" is the same signal without schema change. The remembered value is intentionally left `postcodeTouched = false` so a geotagged photo can still correct it to this space's real location and the auto-geolocate below still knows the user hasn't typed one.
- **Auto-geolocate on the review step (`scan-form.tsx` + `use-locate-postcode.ts`):** a new scan that reaches `review` with an empty postcode (nothing remembered, no photo GPS) fires `navigator.geolocation` once automatically via a guarded effect — no tap. `useLocatePostcode.locate()` gained a `{ silent?: boolean }` option: the automatic attempt passes `silent: true` so a denied permission or no-match degrades with **no** error toast (the user didn't ask for it), while the explicit "Use my location" button keeps its toasts. Fires the permission prompt at most once per mount.
- **Precedence:** remembered → photo GPS → auto-geolocate → "Use my location" button / manual entry. The button still shows only while the postcode is empty, so a successful auto-fill hides it.
- **Tests:** `scan-form.test.tsx` — the location test now asserts the no-tap auto-fill on the review step; added a remembered-postcode test (pre-fills + "from your last space" hint + no geolocation call); the denial test still covers the explicit button. New co-located `src/hooks/use-locate-postcode.test.ts` covers success, silent-denial (no toast), and explicit-denial (toast). Full suite **251/251 green**, `tsc` + `npm run lint` clean.
- **Status:** complete and live-ready — no env vars, no DB change, no ops steps. Ships on the next push to `main`.
