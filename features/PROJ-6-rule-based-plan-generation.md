...
- **`findConstraintViolations({ plants, sun, zone, areaSqm })`** (`src/lib/plan-engine.ts`) — a pure check that re-derives whether every plant in a plan clears the hard survival filters (sun tolerance, winter hardiness zone when known, physical fit). It deliberately does **not** call `matchingSurvivors`, so it can't hide behind the same code that produced the plan. Takes the plants + three site values directly (not a full `GeneratedPlan`) so it checks **both** a freshly generated plan and a persisted one read back from the DB. A clean plan returns `[]`.
...
- **Dev-only runtime assertion** (`src/app/scans/[id]/plan/page.tsx`) — when `NODE_ENV !== 'production'`, the plan view runs the guardrail over the **persisted** plant lines and `console.error`s any violation. This catches an unsurvivable plant that reached the *stored* plan via a catalogue change or a delete-reassignment swap (not just at generation time). Logs only; never blocks render.
- **Where it runs:** `npm test` / CI (the matrix) + dev render (the persisted-plan check). No DB or schema change, no production runtime behaviour change — purely a regression net around the survival promise.
