...
- **BUG-7 (Critical) — FIXED. Found by the two-account harness.** The `authenticated` and `service_role` roles had **no table-level privileges** on `public.users`, so every authenticated profile read/write failed at runtime with `42501: permission denied for table users` — independent of RLS (which only narrows access once a base GRANT exists). Supabase's default privileges hadn't applied to the table (created via the SQL editor under an uncovered role). Undetectable until SMTP enabled the first real authenticated session. **Fix:** `supabase/migrations/20260618140000_proj2_grant_users_privileges.sql` — `grant select, insert, update, delete on public.users to authenticated; grant all to service_role`. Applied to the live DB + verified via `has_table_privilege` (`public.users` only). ⚠️ **`public.scans` has the identical gap and is NOT yet fixed** — its grant + migration are owed on `proj-3-spec`, so PROJ-3 scan reads/writes still fail at runtime until applied. **New convention:** every user-data table needs an explicit `GRANT ... TO authenticated`; don't rely on default privileges.
